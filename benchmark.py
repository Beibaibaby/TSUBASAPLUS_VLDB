...
def low_list_gen(thre_u,thre_l, mean_low_triger, std_low_triger, d_low, d_high):
    x_energy = np.random.normal(mean_low_triger,std_low_triger,d_low)
    D = math.sqrt(1 - thre_u)
    D_low= math.sqrt(1 - thre_l)
    y_energy = np.random.normal(mean_low_triger+1,std_low_triger,d_low)
    while np.linalg.norm(x_energy-y_energy) > D or np.linalg.norm(x_energy-y_energy)<D_low:
        y_energy = np.random.normal(mean_low_triger+1, std_low_triger, d_low)
...
x_low, y_low = low_list_gen(thre_e,0.8, mu_mean, std_mean, d_low, d_high) # a pair of sample
print(x_low)
print(y_low)
